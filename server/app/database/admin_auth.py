...
_DEVELOPMENT_SESSION_SECRET = "development-only-openpaper-admin-session-secret"
...
class AdminAuthenticationBackend(AuthenticationBackend):
    """Authenticate SQLAdmin with shared identity and OpenPaper product role."""

    @staticmethod
    def _is_openpaper_admin(user_id: int) -> bool:
    ...
    async def login(self, request: Request) -> bool:
        ...
        try:
            is_admin = await asyncio.to_thread(self._is_openpaper_admin, user.id)
        except Exception:
        ...
        request.session["openpaper_admin_user_id"] = user.id
        request.session["openpaper_admin_session_id"] = session_id
        return True

    async def logout(self, request: Request) -> bool:
        user_id = request.session.get("openpaper_admin_user_id")
        session_id = request.session.get("openpaper_admin_session_id")
    ...
    async def authenticate(self, request: Request) -> bool:
        ...
        if not await auth_manager.touch_session(user_id, session_id):
            request.session.clear()
            return False
        is_admin = await asyncio.to_thread(self._is_openpaper_admin, user_id)
        if not is_admin:
            await auth_manager.logout(user_id, session_id)
            request.session.clear()
        return is_admin
