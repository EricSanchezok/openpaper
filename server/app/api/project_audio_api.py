...
from app.schemas.orm_responses import (
    serialize_audio_overview,
    serialize_audio_overview_job,
)
...
async def create_project_audio_overview(
    ...
    access = get_project_access(db, project_id=project_uuid, user_id=current_user.id)
    if access is None:
...
async def get_audio_overview_jobs_by_project_id(
    ...
    audio_overview_job_list = [
        serialize_audio_overview_job(job) for job in audio_overview_jobs
    ]
