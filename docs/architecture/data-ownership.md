...
OpenPaper shares one PostgreSQL database named `sanchezcloud` with the other
SanchezCloud products. Schema ownership, not database separation, defines the
service boundary:

| Repository | Owns | Must not own |
| --- | --- | --- |
| `cloud-auth` | `auth.users`, `auth.refresh_tokens`, `auth.schema_migrations`; shared identity, credentials, global account security, product-scoped sessions | subscriptions, quotas, usage, product roles or product blocks |
| `scholight` | `scholight.*`; search product state, quota, usage, keys and history; its arXiv/Zilliz search pipeline | auth migrations or OpenPaper data |
| `openpaper` | `openpaper.*`; documents, projects, product profile/admin/block state, subscriptions and product usage | auth migrations or Scholight data |
...
`scholight_migrator`, or `openpaper_migrator`). Runtime roles receive only the
...
OpenPaper validates the installed `auth.schema_migrations` version before
running its single clean baseline. It never bundles or executes cloud-auth
migrations.
...
5. Use cloud-auth's `UserManager` session API and never query
...
OpenPaper does not use or administer Scholight's Zilliz collections.
