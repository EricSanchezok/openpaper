...
RELEASE_ENV=${SCHOLENS_RELEASE_ENV:?SCHOLENS_RELEASE_ENV is required}
...
domain=$(sed -n 's/^SCHOLENS_DOMAIN=//p' "${RUNTIME_ENV}")
[[ -n ${domain} ]] || { printf 'SCHOLENS_DOMAIN is missing\n' >&2; exit 1; }
retry 20 curl --fail --silent --show-error --max-time 10 "https://${domain}/healthz" \
  >/dev/null

printf 'Scholens smoke checks passed\n'
